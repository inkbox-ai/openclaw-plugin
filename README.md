...
Keep that process running. On startup the plugin opens an Inkbox tunnel, ensures mail/text webhook subscriptions, configures phone call delivery, and routes inbound email, SMS, and calls into OpenClaw sessions.
...
After the gateway prints `[gateway] ready`, `[inkbox] tunnel open`, mail/text webhook subscription active, and phone call delivery set:
...
- Tunnel-first inbound: with a signing key, gateway opens an Inkbox tunnel, syncs mail/text webhook subscriptions, and configures phone call delivery.
